...
SYNOPSIS
    ...
    detectdash [-h,--help] [-v,--verbose] [--version]
...
def logwrite(message):
    ...
    logfile.write(message + ' ' + datetime.datetime.utcnow().isoformat() + '\n')
    logfile.close
    ...
    if pkt.haslayer(DHCP):
        if pkt[Ether].src == '44:65:0d:8e:93:48':
            if time.time() - lastdetection > 10:
                ...
                logwrite("Ariel Button     ")
        else:
            logwrite(pkt[Ether].src)
...
def main():
    ...
    sniff(prn=arp_display, filter="(udp and (port 67 or 68))", store=0)
...
if __name__ == '__main__':
    try:
        ...
        parser.add_option ('-m', '--message', dest='message', default='Hello, World!', type='string', help='Message$
        parser.add_option ('-v', '--verbose', action='store_true', default=False, help='verbose output')
    (options, args) = parser.parse_args()
        if options.verbose: logwrite(datetime.utcnow().isoformat())
        main()
        if options.verbose: logwrite(datetime.utcnow().isoformat())
        if options.verbose: logwrite('TOTAL RUNNING TIME IN MINUTES:'),
        if options.verbose: logwrite((time.time() - start_time) / 60.0)
        sys.exit(0)
